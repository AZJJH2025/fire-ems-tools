/**
 * NFIRS Validator - Converted from module to regular script
 */
...
function validateNFIRSCompliance(incident) {
    ...
    const requiredFields = [
        {
            ...
            validate: (value) => value && window.NFIRS_CODES.incidentTypes[value]
        },
        {
            field: "property_use",
            path: "incident_type.property_use",
            message: "NFIRS Property Use is required",
            validate: (value) => value && window.NFIRS_CODES.propertyUse[value]
        },
        {
            field: "address",
        ...
        },
        {
            ...
            validate: (value) => value && window.NFIRS_CODES.aidGivenReceived[value]
        }
    ];
    ...
    if (!incident.actions || !incident.actions.length) {
        ...
        incident.actions.forEach((action, index) => {
            if (!action.code || !window.NFIRS_CODES.actionTaken[action.code]) {
                errors.push({
    ...
    if (isStructureFire) {
        if (!incident.detector || !window.NFIRS_CODES.detector[incident.detector]) {
            errors.push({
                field: "detector",
...
function getNFIRSCodeLabel(code, codeType) {
    if (!code || !codeType || !window.NFIRS_CODES[codeType]) {
        return code || '';
    }
    
    return window.NFIRS_CODES[codeType][code] || code;
}
...
function searchNFIRSCodes(searchTerm, codeType) {
    if (!searchTerm || !codeType || !window.NFIRS_CODES[codeType]) {
        return [];
    }
    ...
    const results = [];
    const codes = window.NFIRS_CODES[codeType];
...
window.IncidentLogger.NFIRS.Validator = {
    ...
    isReadyForExport: isNFIRSReadyForExport,
    getMissingFields: getMissingNFIRSFields,
    formatFieldValue: formatNFIRSFieldValue,
    getCodeLabel: getNFIRSCodeLabel,
    formatCodeDisplay: formatNFIRSCodeDisplay,
    searchCodes: searchNFIRSCodes
};

console.log("NFIRS Validator loaded successfully (fixed version)");