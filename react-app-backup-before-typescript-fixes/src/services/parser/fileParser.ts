    ...
    reader.onload = async (e) => {
      try {
        ...
        try {
          ...
          // Try to detect if the first line contains column headers
          // This is a best effort approach - PDF parsing is inherently messy
          const potentialHeaders = lines[0].split(/\s{2,}/).map(h => h.trim()).filter(h => h);
          const firstDataLine = lines[1]?.split(/\s{2,}/).map(cell => cell.trim()).filter(cell => cell) || [];
          
          let columns: string[];
          let startIndex: number;
          
          // If the first line has multiple fields and seems like a header, use it
          if (potentialHeaders.length > 1 && potentialHeaders.length === firstDataLine.length) {
            columns = potentialHeaders;
            startIndex = 1; // Skip the header row
          } else {
            // Otherwise, generate column names
            columns = ['Column1', 'Text'];
            startIndex = 0;
          }
          ...
          if (columns.length === 1 || columns[0] === 'Text') {
            // If we just have text content without clear columns,
            // treat each line as a single row with one column
            for (let i = startIndex; i < Math.min(lines.length, 100); i++) {
              data.push({
                'Text': lines[i]
              });
            }
          } else {
            // If we have detected columns, try to parse structured data
            for (let i = startIndex; i < Math.min(lines.length, 100); i++) {
              const lineParts = lines[i].split(/\s{2,}/).map(part => part.trim()).filter(part => part);
              const rowData: Record<string, any> = {};
              
              for (let j = 0; j < columns.length; j++) {
                rowData[columns[j]] = lineParts[j] || '';
              }
          ...
          // Fallback if pdf-parse import or parsing fails
          console.error('PDF parsing failed:', pdfError);
          
          // Provide basic representation
          resolve({
            columns: ['Text'],
            data: [{ 'Text': 'PDF parsing failed - this file type may not be supported in the browser' }]
          });
        }
      } catch (error) {
