...
import { applyLayoutTemplate } from '@/state/redux/fireMapProSlice/index';
...
const LayoutToolbox: React.FC<LayoutToolboxProps> = ({ 
  ...
  const applyTemplate = (templateId: LayoutTemplate) => {
    ...
    // Template layouts handled by Redux action
    dispatch(applyLayoutTemplate(templateId));
  };

  // Note: Template definitions moved to Redux reducer for proper state management
  /*
    ...
    <Box sx={{ p: 2 }}>
      ...
      <Grid container spacing={1} sx={{ mb: 3 }}>
        {layoutElements.map(element => (
          <Grid size={{ xs: 6 }} key={element.type}>
            <Paper
              sx={{
                p: 1,
                textAlign: 'center',
