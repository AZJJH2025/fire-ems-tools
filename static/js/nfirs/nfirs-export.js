/**
 * FireEMS.ai Incident Logger - NFIRS Export Module
 * 
 * This module provides functionality for exporting incident data
 * in NFIRS-compatible formats.
 */

import { isNFIRSReadyForExport, getMissingNFIRSFields } from './nfirs-validator.js';
...
function convertToNFIRSXML(incident) {
    ...
    if (!isNFIRSReadyForExport(incident)) {
        throw new Error("Incident is missing required NFIRS fields: " + 
                       getMissingNFIRSFields(incident).join(", "));
    }
...
function convertToNFIRSJSON(incident) {
    ...
    const nfirsData = {
        ...
        basic: {
            ...
            incident_type: {
                ...
                description: NFIRS_CODES.incidentTypes[incident.incident_type?.primary] || ""
            },
            property: {
                property_use: {
                    code: incident.incident_type?.property_use || "NNN",
                    description: NFIRS_CODES.propertyUse[incident.incident_type?.property_use] || "None"
                }
            },
            address: {
            ...
            aid: {
                ...
                description: NFIRS_CODES.aidGivenReceived[incident.aid_given_received] || "None"
            },
            actions_taken: []
        }
    };
    ...
    if (incident.actions && incident.actions.length > 0) {
        incident.actions.forEach(action => {
            nfirsData.basic.actions_taken.push({
                code: action.code,
                description: NFIRS_CODES.actionTaken[action.code] || ""
            });
        });
    }
    ...
    if (incident.detector) {
        nfirsData.basic.detector = {
            ...
            description: NFIRS_CODES.detector[incident.detector] || ""
        };
    }
