...
const PreviewValidationContainer: React.FC = () => {
  ...
  const [highlightedCell, setHighlightedCell] = useState<{ rowIndex: number; field: string } | null>(null);
  ...
  const handleJumpToError = (rowIndex: number, field: string) => {
    ...
    setHighlightedCell({ rowIndex, field });

    // Scroll to the table after a short delay to ensure tab switch completes
    setTimeout(() => {
      if (tableRef.current) {
  ...
  const handleJumpToMapping = (_fieldName: string) => {
    // Field highlighting simplified - direct navigation only
