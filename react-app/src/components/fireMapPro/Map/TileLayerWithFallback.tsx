...
const TileLayerWithFallback: React.FC<TileLayerWithFallbackProps> = ({
  primary,
  fallbacks = []
  // onError - removed for simplification
}) => {
  const [currentProviderIndex, setCurrentProviderIndex] = useState(0);
  // const [hasError, setHasError] = useState(false); // Simplified error handling
  ...
  // Note: Event handlers simplified for compatibility
  /*
  ...
  */
  ...
  return (
    <TileLayer
      ...
      minZoom={(currentProvider as any).minZoom || 1}
      ...
      // Note: onLoad and onLoading events removed for compatibility
      // Error handling done through retry mechanism
    />
  );
};
