    ...
    if (mapping.transformations && mapping.transformations.length > 0) {
      mapping.transformations.forEach(transform => {
        switch (transform.type) {
          case 'format':
            if (transform.params.dateFormat) {
              if (transform.params.dateFormat === 'custom') {
                setDateFormat('custom');
                setCustomDateFormat(transform.params.customFormat || '');
              } else {
                setDateFormat(transform.params.dateFormat);
              }
            }
            break;
          case 'split':
        ...
        <Box sx={{ width: '100%' }}>
          ...
          <Box role="tabpanel" hidden={tab !== 0} sx={{ p: 2 }}>
            ...
            <Typography variant="body2" color="text.secondary" paragraph>
              Choose how date and time values should be formatted.
            </Typography>
