...
const PureLeafletDrawingTools: React.FC<PureLeafletDrawingToolsProps> = ({ map }) => {
  ...
  const createFeatureFromEvent = (event: any, type: FeatureType): MapFeature => {
    ...
    return {
      ...
      coordinates,
      style: {
        color: drawingState.options.strokeColor,
        fillColor: drawingState.options.fillColor,
        fillOpacity: drawingState.options.fillOpacity,
        weight: drawingState.options.strokeWidth,
        opacity: drawingState.options.strokeOpacity
      },
      properties: {
    ...
    const startDrawing = (e: L.LeafletMouseEvent) => {
      ...
      switch (drawingModeRef.current) {
        case 'marker':
          ...
          dispatch(addFeature(markerFeature));
        ...
        case 'rectangle':
          ...
          tempLayer = L.rectangle([e.latlng, e.latlng]);
          tempLayer.addTo(map);
          break;
      }
    };
