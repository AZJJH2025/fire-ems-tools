...
const TargetFieldsPanel: React.FC<TargetFieldsProps> = ({
  ...
  const renderTargetField = (field: TargetField) => {
    ...
    const mappedSourceField = targetFieldMappings[field.name];
    // For validation feedback
    const validation = fieldValidations[field.name];

    // Find the full mapping object for this field (needed for transformations)
    const fullMapping = mappings.find(m => m.targetField === field.name);

    // Check if this field is selected for preview
    const isSelected = selectedFieldForPreview === field.name;

    // Debug logging to help identify mapping issues
    console.log(`Rendering field ${field.name}, mapped to: ${mappedSourceField || 'none'}`);
                ...
                >
                  <IconButton
                    ...
                    onClick={(e) => {
                      ...
                      handleTransformSettingsOpen(field.name);
                    }}
                    sx={{ mr: 1 }}
                  >
        ...
        <FormControl fullWidth size="small">
          ...
          <Select
            ...
            onChange={(e) => {
              ...
              try {
                handleMappingDropdownChange(field.name, e.target.value);
