...
const MapArchitectureTest: React.FC = () => {
  ...
  const [, setTestMapInstance] = useState<L.Map | null>(null);
  ...
  const logLifecycle = useCallback((event: string, details?: any) => {
    const log: LifecycleLog = {
      timestamp: Date.now(),
      event,
      mapId: mapInstanceRef.current ? `map_${(mapInstanceRef.current as any)._leaflet_id}` : undefined,
      details
    };
  ...
  const testMapPersistence = useCallback(async () => {
    ...
    try {
      ...
      const mapId = `map_${(map as any)._leaflet_id}`;
